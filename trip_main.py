...
def get_places(age, nationality, museum_type):
    places = []
    data = pd.read_csv('museums2.csv')

    if museum_type != 'all':
        data = data[data['type'] == museum_type.lower()]
    # print(f'[DEBUG] Data: {data}')

    # Set the price index
    if age == 'adult' and nationality == 'egyptian':
        global_price_index = 10    # 'egyptian and arab'
    elif age == 'student' and nationality == 'egyptian':
        global_price_index = 11    # 'egyptian and arab student'
    elif age == 'adult' and nationality == 'foreigner':
        global_price_index = 12    # 'foreigner'
    elif age == 'student' and nationality == 'foreigner':
        global_price_index = 13    # 'foreigner student'
    else:
        global_price_index = 12    # 'foreigner'
...
def find_population(lat, lon, startTime, time, budget, age, nationality, museum_type):
    if startTime < 9:
        startTime = 9
    ...
    places = get_places(age, nationality, museum_type)
...
if __name__ == '__main__':
    ...
    result = find_population(lat, long, 999, time, budget, "adult", "egyptian", "type")
    print(result)
